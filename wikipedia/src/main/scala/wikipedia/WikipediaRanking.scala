...
object WikipediaRanking {
    ...
    val conf: SparkConf = new SparkConf().setAppName("wiki").setMaster("local")
    ...
    def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
        rdd.aggregate(0)((sum, article: WikipediaArticle) => if (article.mentionsLanguage(lang)) sum + 1 else sum,
            (sum1, sum2) => sum1 + sum2)
    }
    ...
    def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
        rdd.flatMap(article => langs.filter(lang => article.mentionsLanguage(lang)).map((_, 1))).reduceByKey(_ + _).sortBy(-_._2).collect().toList
    }
