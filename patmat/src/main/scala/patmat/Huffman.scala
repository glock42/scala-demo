...
object Huffman {
    ...
    def chars(tree: CodeTree): List[Char] = tree match {
        ...
        case Fork(left,right,chs,weight) => chars(left) ::: chars(right)
    }
    ...
    def times(chars: List[Char]): List[(Char, Int)] = {
       countTimes(chars,Nil)
    }
    def countTimes(chars:List[Char],pairs:List[(Char, Int)]):List[(Char, Int)] = {
        if(chars == Nil) pairs
        else  countTimes(chars.tail,getPairs(chars.head,pairs))
    }
    def getPairs(char:Char, pairs:List[(Char, Int)]):List[(Char, Int)] = {
        if (pairs == Nil) List((char,1))
        else if(char == pairs.head._1) List((pairs.head._1,pairs.head._2+1)) ::: pairs.tail
        else List((pairs.head._1,pairs.head._2)):::getPairs(char,pairs.tail)
    }
    ...
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
        if (freqs == List()) List()
        else insert(freqs.head,makeOrderedLeafList(freqs.tail))
    }

    def insert(pair: (Char, Int), pairs: List[Leaf]): List[Leaf] = {
        if (pairs == List()) List(Leaf(pair._1,pair._2))
        else if (pair._2 <= pairs.head.weight) List(Leaf(pair._1,pair._2)) ::: pairs
        else List(pairs.head) ::: insert(pair,pairs.tail)
    }
